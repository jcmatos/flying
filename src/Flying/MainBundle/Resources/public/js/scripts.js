...
//ids para envio do email
var email_go_id1;
var email_go_id2;
var email_return_id1;
var email_return_id2;


var sel_lang; // lingua usada
var url_to_update; // link enviado para o metodo que faz os requests novamente, contendo os parametros da procura feita
...
var direct_flight; // indica se e' uma procura de voo directo ou com escala

function set_direct_flight(direct_flight) {
...
function get_airport_list_text(code) {
...
function set_defaults(min_wait_time, max_wait_time, max_flight_price, max_trip_price, result_limit, min_return_days, max_return_days, max_tolerate_wait, sel_lang, url_to_update) {
	...
	this.url_to_update = url_to_update;
}
...
function get_trips(search_start_date, search_end_date, returning, date_of_going_flight) {
	...
	if (!returning) temp_flights1=all_flights_go1;
		...
		if (!direct_flight)	temp_flights1 = all_flights_return1
		else temp_flights1 = all_flights_return2;
	}
	...
	for (fl1=0; fl1 < temp_flights1.length; fl1++ ) {
		...
		if (temp_flights1[fl1].date >= search_start_date && temp_flights1[fl1].date <= search_end_date && temp_flights1[fl1].price <= max_flight_price) {
			if (!direct_flight) check_second_flight(temp_flights1[fl1], returning, date_of_going_flight);
			else check_second_flight_direct(temp_flights1[fl1], returning, date_of_going_flight);
		}
	}
...
// voos com escala
function check_second_flight_direct(check, returning, date_of_going_flight) {

	if (check.price <= max_flight_price) {
		...
		'<span class="glyphicon glyphicon-refresh res_refresh_btn" data-id1="'+check.id+'"></span>'+
		'<span class="res_total"><span class="glyphicon glyphicon-credit-card"></span> &euro; '+flight1_price_string+'</span>'+
		'</span>'+
		'</div>'+
		'<div class="col-md-6 res_flight1">'+
		'<span class="res_date1"><span class="glyphicon glyphicon-calendar"></span>'+check.date+'</span>'+
		'<span class="res_origin1"><span class="glyphicon glyphicon-upload"></span>'+airport_list[check.origin]+'</span>'+
		'<span class="res_destination1"><span class="glyphicon glyphicon-download"></span>'+airport_list[check.destination]+'</span>'+
		'<span class="res_departure1"><span class="glyphicon glyphicon-export"></span>'+check.departure+'</span>'+
		...
		var one_result = new one_flight_result(code_to_add, flight1_price);

		if (flight_results.length == 0) {
			flight_results.push(one_result);
		}
		else /*if (flight1_price <= max_flight_price)*/ {
			var inserted=false;
			for (i=0; i<flight_results.length && inserted==false; i++ ) {
				
				if (parseFloat(flight1_price)<=parseFloat(flight_results[i].price)) {
...
function format_date(date_to_format) {
	...
	if(sel_lang=='pt') {
		...
		if (day_name==0) day_name = 'Sunday';
		else if (day_name==1) day_name = 'Monday';
		else if (day_name==2) day_name = 'Tuesday';
		else if (day_name==3) day_name = 'Wednesday';
		else if (day_name==4) day_name = 'Thursday';
		else if (day_name==5) day_name = 'Friday';
		else if (day_name==6) day_name = 'Saturday';

		if (month_name==0) day_name = 'January';
		else if (month_name==1) month_name = 'February';
		else if (month_name==2) month_name = 'March';
		else if (month_name==3) month_name = 'April';
		else if (month_name==4) month_name = 'May';
		else if (month_name==5) month_name = 'June';
		else if (month_name==6) month_name = 'July';
		else if (month_name==7) month_name = 'August';
		else if (month_name==8) month_name = 'September';
		else if (month_name==9) month_name = 'October';
		else if (month_name==10) month_name = 'November';
		else if (month_name==11) month_name = 'December';
		

		return day_name + ', ' + month_name + ' ' + date_to_format.getDate();	
	}
}
